...
class Disk(Geometry):
    ...
    def on_boundary(self, x):
        return np.isclose(np.linalg.norm(x - self.center, axis=-1), self.radius)    #不明白这里为什么有一个axis = -1

    def distance2boundary_unitdirn(self, x, dirn):  #直线的方向向量为dirn，这里为未单位化后的方向向量
        """https://en.wikipedia.org/wiki/Line%E2%80%93sphere_intersection"""
        xc = x - self.center
        ad = np.dot(xc, dirn)
        return -ad + (ad ** 2 - np.sum(xc * xc, axis=-1) + self._r2) ** 0.5

    def distance2boundary(self, x, dirn):       #直线的方向向量为dirn，这里为单位化后的方向向量，这两个函数名称是不是反了？
    ...
    def boundary_normal(self, x):  #目的是求x所在点的法向量
        _n = x - self.center
    ...
    def uniform_boundary_points(self, n):  #输出为一个横向量，前一半为x的坐标，后一半为y的坐标
    ...
    def background_points(self, x, dirn, dist2npt, shift):  #不明白
...
class Rectangle(Hypercube):
    ...
    def random_boundary_points(self, n, random="pseudo"):  #可以取到左下角和右上角，但是取不到左上角和右下角
...
class Triangle(Geometry):
    ...
    def boundary_normal(self, x):  #求x所在边的法向量
...
class Polygon(Geometry):
    ...
    def __init__(self, vertices):
        ...
        self.segments = np.vstack((self.vertices[0] - self.vertices[-1], self.segments))   #表示边的向量
        self.normal = clockwise_rotation_90(self.segments.T).T   #求边长的法向量
        self.normal = self.normal / np.linalg.norm(self.normal, axis=1).reshape(-1, 1)  #归一化

    def inside(self, x):
        def wn_PnPoly(P, V):    #这里用了一个比较有趣的算法，winding number algorithm
    ...
    @vectorize(excluded=[0], signature="(n)->(n)")   #好像是向量化提升计算速度？
    ...
    def random_points(self, n, random="pseudo"):  #这里没看懂，这难道不是都取在对角线上了嘛
